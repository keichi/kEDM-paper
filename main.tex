...
\begin{algorithm}
    ...
    \KwOut{Top-$k$ distance matrix $D_k$ and index matrix $I_k$}
    \tcp{\texttt{TeamPolicy}}
    \PFor{$i \leftarrow 1$ \KwTo $L$}{
        \tcp{\texttt{ThreadRange}}
        \PFor{$j \leftarrow 1$ \KwTo $L$}{
            Insert $D(i, j)$ and $j$ into local priority queue\;
        }
        \For{$j \leftarrow 1$ \KwTo $k$}{
            \For{$j \leftarrow 1$ \KwTo \# of teams}{
                Insert $D(i, j)$ and $j$ into local priority queue\;
                $D_k(i, j) \leftarrow$ Pop element from priority queue\;
            }
        }
    }
