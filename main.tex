...
\begin{enumerate}
    ...
    \item For each point $x(t)$ in $x$, $E+1$ nearest neighbors of $x(t)$ in
        the state space are found. Suppose the nearest neighbors are $x(t_1),
        x(t_2), \dots, x(t_{E+1})$ and the Euclidean distance between $x(t_i)$
        and $x(t)$ is $d(t_i)$.
    \item Prediction $\hat{y}(t)$ for $y(t)$ is a linear combination of
        corresponding points $y(t_1), y(t_2), \dots, y(t_{E+1})$.
        Specifically,
        \begin{equation}
            \hat{y}(t) = \sum^{E+1}_{i=1} \frac{\exp{d(t_i)}}{\sum
            \exp{d(t_i)}} \cdot y(t_i)
        \end{equation}
        The first component of $\hat{y}(t)$ is $\hat{Y}(t)$.
    \item Pearson's correlation $\rho$ between $Y$ and $\hat{Y}$ is used to
        assess the predictive skill. If $\rho$ is high, we conclude $Y$
        ``CCM-causes'' $X$.
\end{enumerate}
...
\begin{algorithm}
    \SetAlgoLined
    \DontPrintSemicolon
    \KwIn{time series $x$, embedding dimension $E$, time lag $\tau$}
...
\begin{algorithm}
    ...
    \KwOut{Partially sorted distance matrix}
    \PFor{$i \leftarrow 1$ \KwTo $L$}{
...
\begin{algorithm}
    \SetAlgoLined
    \DontPrintSemicolon
    \KwIn{target time series, distances table, indices table, embedding
    dimension $E$} \KwOut{Pairwise distances}
    \PFor{$i \leftarrow 1$ \KwTo $N$}{
        \PFor{$j \leftarrow 1$ \KwTo $L$}{
            $p \leftarrow 0$\;
            \For{$k \leftarrow 1$ \KwTo $E+1$}{
                $p \leftarrow p + d(j, k) \cdot y(t(j, k))$
            }
            $\hat{y}(i, j) \leftarrow p$\;
        }
    }
    \caption{Lookup}%
    \label{alg:lookup}
