...
\begin{algorithm}
    \SetAlgoLined
    \DontPrintSemicolon
    \KwIn{library time series, embedding dimension $E$, time lag $\tau$}
    ...
    \PFor{$i \leftarrow 1$ \KwTo $L$}{
        \PFor{$j \leftarrow 1$ \KwTo $L$}{
            ...
            \For{$k \leftarrow 1$ \KwTo $E$}{
                $\mathrm{distances}(i, j) \leftarrow \mathrm{distances}(i, j) + (\mathrm{library}(k \tau + i) - \mathrm{library}(k \tau + j))^2$\;
            }
        }
    }
    ...
    \PFor{$i \leftarrow 1$ \KwTo $N$}{
        \PFor{$j \leftarrow 1$ \KwTo $L$}{
            ...
            \For{$k \leftarrow 1$ \KwTo $E+1$}{
                $p \leftarrow p + \mathrm{distances}(j, k) \cdot \mathrm{target}
                (\mathrm{indices}(j, k))$
            }
            $\mathrm{prediction}(i, j) \leftarrow p$\;
        }
    }
    \caption{Lookup}%
    \label{alg:lookup}
